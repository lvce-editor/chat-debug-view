...
export const name = 'chat-debug-view.table-header-toggle-method'

export const skip = 1

export const test: Test = async ({ ChatDebug, Command, ContextMenu, expect, Locator }) => {
  const sessionId = 'e2e-session-table-header-toggle-method'
  ...
  await ContextMenu.selectItem('Method')
  ...
  await expect(hiddenHeaderCellsNth1).toHaveText('Status')
  const hiddenHeaderCellsNth2 = headerCells.nth(2)
  await expect(hiddenHeaderCellsNth2).toHaveText('Size')
  const hiddenHeaderCellsNth3 = headerCells.nth(3)
  await expect(hiddenHeaderCellsNth3).toHaveText('Time')

  await Command.execute('ChatDebug.handleHeaderContextMenu', 0, 300)
  await ContextMenu.selectItem('Method')

  await expect(headerCells).toHaveCount(5)
  const methodHeader = headerCells.nth(1)
  await expect(methodHeader).toHaveText('Method')
