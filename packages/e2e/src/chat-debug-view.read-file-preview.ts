...
export const test: Test = async ({ ChatDebug, Command, expect, Locator }) => {
  ...
  const lineNumbersNth1 = lineNumbers.nth(1)
  await expect(lineNumbersNth1).toHaveText('2')
  await expect(lineContents).toHaveCount(2)
  const lineContentsNth0 = lineContents.nth(0)
  await expect(lineContentsNth0).toHaveText('first line')
  const lineContentsNth1 = lineContents.nth(1)
  await expect(lineContentsNth1).toHaveText('second line')
