...
export const test: Test = async ({ ChatDebug, expect, Locator }) => {
  ...
  const lineNumber23 = lineNumbers.nth(23)
  await expect(lineNumber23).toHaveText('24')
  await expect(lineContents).toHaveCount(24)
  const lineContent0 = lineContents.nth(0)
  await expect(lineContent0).toHaveText('line 1')
  const lineContent23 = lineContents.nth(23)
  await expect(lineContent23).toHaveText('line 24')
}
