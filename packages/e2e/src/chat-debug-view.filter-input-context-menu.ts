...
export const test: Test = async ({ ChatDebug, expect, KeyBoard, Locator }) => {
  ...
  await expect(menuItemsNth0).toHaveText('Cut')
  const menuItemsNth1 = menuItems.nth(1)
  await expect(menuItemsNth1).toHaveText('Copy')
  const menuItemsNth2 = menuItems.nth(2)
  await expect(menuItemsNth2).toHaveText('Paste')
  const menuItemsNth3 = menuItems.nth(3)
  await expect(menuItemsNth3).toHaveText('Select All')
