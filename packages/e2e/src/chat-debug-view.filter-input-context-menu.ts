...
export const test: Test = async ({ ChatDebug, expect, KeyBoard, Locator }) => {
  ...
  await expect(menuItem0).toHaveText('Cut')
  const menuItem1 = menuItems.nth(1)
  await expect(menuItem1).toHaveText('Copy')
  const menuItem2 = menuItems.nth(2)
  await expect(menuItem2).toHaveText('Paste')
  const menuItem3 = menuItems.nth(3)
  await expect(menuItem3).toHaveText('Select All')
