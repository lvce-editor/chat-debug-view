...
export const name = 'chat-debug-view.headers-request-collapse'

export const test: Test = async ({ ChatDebug, Command, expect, Locator }) => {
  const sessionId = `e2e-session-headers-request-collapse-${Date.now()}`
  ...
  const requestHeadersHeading = Locator('.ChatDebugViewHeadersSectionHeading[value="requestHeaders"]')
